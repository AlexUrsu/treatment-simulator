...
type
  ...
  //сферы психики
    ...
    sverhcenniost: integer;     // отношение к идеям
    bred_sistema: integer;     // степень систематизации идеи (бреда)
    ...
    ответсвенность в плане выполнения процедур

    удовлетворенность работой
    удовлетворенность семейными отношениями
  ...
  const
    UM_VIR: string ='незначительная ';
    DO_VIR: string ='умеренная ';
    VV_VIR: string ='выраженная ';

implementation
uses Unit1;

  function interpreracia1 (x: integer; s: string): string;
     // критерии +-50 = норма, 50-65 умерено выражено, 65-85 акцентуация, >85 явно выражено
    var
      s1: string;
    begin
      if x<=50 then
...
//    self.zrelost_infantilnost:=random(200)-100;
  ...
  var
    ...
    i: integer;
  begin
    rg:=interpreracia1(self.rigidnost_labilnost, 'ригидность; ');
    lab:=interpreracia2(self.rigidnost_labilnost, 'эмоциональная лабильность; ');
    hr:=interpreracia1(self.hypertymia_hypotymia, 'гипертимия; ');
    hy:=interpreracia2(self.hypertymia_hypotymia, 'гипотимия; ');
    agr:=interpreracia1(self.agress_trevoga, 'эксплозивность; ');
    trev:=interpreracia2(self.agress_trevoga, 'тревожность; ');
    orig:=interpreracia1(self.original_poryadok, 'оригинальность; ');
    poryad:=interpreracia2(self.original_poryadok, 'педантичность; ');
//    ivent:=interpreracia1(self.introvert_extravert, 'интроверсия; ');
//    extrvent:=interpreracia2(self.introvert_extravert, 'экстраверсия; ');
//    infantil:=interpreracia2(self.zrelost_infantilnost, 'инфантильность; ');
    if (rg='') and (lab='') and (hr='') and (hy='') and (agr='') and (trev='') and (orig='') and (poryad='') then
    result:='В характере не наблюдается каих-либо особенностей.' else begin
    re:='В характе наблюдается: '+rg+lab+hr+hy+agr+trev+orig+poryad;
    i:=Length(re);
    i:=i-1;
    re[i]:='.';
    result:=re;
...
//    self.sex:=false;
  ...
  begin
    ...
    result:= 'Пациент '+self.name+'. '+IntToStr(self.age)+let else
    result:= 'Пациентка '+self.name+'. '+IntToStr(self.age)+let;
  end;
...
///////////////////////////////////////////////////////////////////////////////////
    // коррекция на возраст органики  !!!!!!!!!!!! отменил
    cor_age:=self.age*0;
...
///////////////////////////////////////////////////////////////////////////////////
  ...
  begin
    ...
    // мышление
    //if (drug.group='нормотимик') and (self.hyper_hypomentzia<0) and (drug.name<>'Лития карбонат') then
    ...
    // память
    //if (drug.group='нормотимик') and (self.hyper_hypomnesia<0) and (drug.name<>'Лития карбонат') then
   ...
   // эмоции
    //if (drug.group='нормотимик') and (self.hyper_hypotimia<0) and (drug.name<>'Лития карбонат') then
    ...
    //if (drug.group='нормотимик') and (self.hyperpatia_apatia<0) and (drug.name<>'Лития карбонат') then
